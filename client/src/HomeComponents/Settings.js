...
function Settings() {
  ...
  async function getBot(){
    const response= await fetch(`http://20.193.147.19:8080/get-bot?key=${document.cookie.slice(4)}`)
  ...
  async function setBotAtServer(){
    if (botSelected!=''){
      const response = await fetch(`http://20.193.147.19:8080/set-bot?key=${document.cookie.slice(4)}&bot=${botSelected}`)
      const data = await response.json()
      if (data=='1'){
  ...
  async function getAPISettings(){
    const response=await fetch('http://20.193.147.19:8080/get-api-settings')
    const data=await response.json()
    console.log(data)
  ...
  async function setAPISettings(){
    ...
    await fetch('http://20.193.147.19:8080/set-api-settings', {
      method: 'POST',
